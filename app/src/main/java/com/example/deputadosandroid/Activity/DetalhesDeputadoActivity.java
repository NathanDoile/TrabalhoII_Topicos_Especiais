...
public class DetalhesDeputadoActivity extends AppCompatActivity {
    ...
    private TextView nome, sigla, cpf, dataNascimento, situacao;

    private ImageView back, foto_perfil;
    ...
    private Deputado parseJson(String responseData) {
        try{
            ...
            nome.setText(format("NOME: %s", dadosObject.getString("nomeCivil")));

            cpf.setText(format("CPF: %s", dadosObject.getString("cpf")));

            dataNascimento.setText(format("DATA NASCIMENTO: %s", dadosObject.getString("dataNascimento")));

            JSONObject ultimoStatus = dadosObject.getJSONObject("ultimoStatus");

            foto = ultimoStatus.getString("urlFoto");

            sigla.setText(format("SIGLA PARTIDO: %s", ultimoStatus.getString("siglaPartido")));

            situacao.setText(format("SITUAÇÃO: %s", ultimoStatus.getString("situacao")));
